...
main(void)
{
	...
	assert(atomic_exchange(&u8,  end_u8 ) == initial_u8);
	assert(atomic_exchange(&u16, end_u16) == initial_u16);
	assert(atomic_exchange(&u32, end_u32) == initial_u32);
	assert(atomic_exchange(&u64, end_u64) == initial_u64);
