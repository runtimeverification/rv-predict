...
class DeclEditorVisitor
  : public RecursiveASTVisitor<DeclEditorVisitor> {
public:
  explicit DeclEditorVisitor(ASTContext *Context, llvm::StringRef InFile)
    : Context(Context) {
    this->InFile = InFile;
  }
...
public:
  explicit DeclEditorConsumer(ASTContext *Context, llvm::StringRef InFile)
...
public:
  virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(
    ...
    return std::unique_ptr<clang::ASTConsumer>(
        new DeclEditorConsumer(&Compiler.getASTContext(), InFile));
  }
};
...
//	CommonOptionsParser OptionsParser(argc, argv, decledCategory);
	...
	if (comp == NULL) {
		errx(EXIT_FAILURE, "Failed to load compilation database\n");
	}
