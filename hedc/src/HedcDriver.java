 ...
 * All rights reserved
 * 
 * @version $Id: HedcDriver.java,v 1.1 2010/06/30 15:07:00 smhuang Exp $
...
public class HedcDriver implements Runnable {
	...
	TES_THREADS_ = sp.getInteger("HedcDriver.THREADS", 0);
	TES_PAUSE_ = sp.getInteger("HedcDriver.PAUSE", 0);
	TES_ITERATIONS_ = sp.getInteger("HedcDriver.ITERATIONS", 0);
	TES_START_ = sp.getString("HedcDriver.START", null);
	TES_END_ = sp.getString("HedcDriver.END", null);
	TES_SOHO_SYNOPTIC_ = sp.getString("HedcDriver.SOHO_SYNOPTIC", "0");
	TES_RAG_ = sp.getString("HedcDriver.RAG", "0");
	TES_MESOLA_ = sp.getString("HedcDriver.MESOLA", "0");
	TES_SACREAMENTAL_ = sp.getString("HedcDriver.SACREAMENTAL", "0");
	TES_WAIT_TIME_ = sp.getString("HedcDriver.WAIT_TIME", "3");
    }
	...
	for (int i=0; i < TES_THREADS_; ++i) {
	    try {
		t[i] = new Thread(new HedcDriver("thread"+i+".log", TES_PAUSE_, TES_ITERATIONS_));
		t[i].start();
	    } catch (Exception e) {
    ...
    public HedcDriver(String name, int pause, int iterations) throws IOException {
