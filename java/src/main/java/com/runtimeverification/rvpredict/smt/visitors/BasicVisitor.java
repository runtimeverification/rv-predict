...
public class BasicVisitor<TResult> implements Visitor<TResult> {
    ...
    public void visit(SMTTerm node) throws Exception {
        SMTTerm<SMTOperation,SMTFormula> term = node;
    ...
    @Override
    public void visit(InterruptionDepthVariable node) throws Exception {
        visit((SMTVariable) node);
    }

    @Override
    public void visit(SignalEnabledOnThreadStartVariable node) throws Exception {
        visit((SMTVariable) node);
    }
