...
class WhirlyListThreadListIterator<E extends HasThread> implements ThreadListIterator<E> {
  private final Map<Integer,List<E>> map = new HashMap<Integer,List<E>>();
  WhirlyListThreadListIterator(final Iterator<? extends E> it) {
    while (it.hasNext()) {
      final E e = it.next();
      if (!map.containsKey(e.getThreadID()))
        map.put(e.getThreadID(), new WhirlyList<E>());
