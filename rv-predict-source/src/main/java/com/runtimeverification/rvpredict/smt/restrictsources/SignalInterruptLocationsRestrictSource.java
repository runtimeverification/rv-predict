...
import com.runtimeverification.rvpredict.smt.restricts.EventCanOccurBetweenGoodEventAndBadEvent;
import com.runtimeverification.rvpredict.smt.restricts.False;
import com.runtimeverification.rvpredict.smt.restricts.Or;
import com.runtimeverification.rvpredict.smt.restricts.SectionOccursBetweenEvents;
import com.runtimeverification.rvpredict.smt.restricts.SignalInterruptsThread;
...
/**
 * Adds restricts that specify where a signal can interrupt a thread. In order for a signal to interrupt a thread,
 ...
 */
public class SignalInterruptLocationsRestrictSource implements RestrictSource {
    ...
    public SignalInterruptLocationsRestrictSource(
    ...
    public ModelRestrict createRestrict() {
        ImmutableList.Builder<ModelRestrict> allSignalRestricts = new ImmutableList.Builder<>();
        eventsByThreadID.keySet().stream()
                ...
                .forEach(interruptingTtid -> {
                    ...
                    allSignalRestricts.add(computeSignalInterruptionRestrict(
                            interruptingTtid,
                            maybefirstEvent.get(), maybeLastEvent.get(),
                            ttidToSignalNumber.apply(interruptingTtid)));
                });
        return new And(allSignalRestricts.build());
    }

    private ModelRestrict computeSignalInterruptionRestrict(
        ...
        ImmutableList.Builder<ModelRestrict> possibleSignalInterruptions = new ImmutableList.Builder<>();
    ...
    private ModelRestrict signalInterruptsAtStart(
    ...
    private ModelRestrict signalInterruptsThreadAtStart(
            int interruptingSignalTtid, long interruptingSignalNumber,
            ReadonlyEventInterface firstInterruptingSignalEvent, ReadonlyEventInterface lastInterruptingSignalEvent,
            Collection<ReadonlyEventInterface> interruptedThreadEvents,
            int interruptedTtid) {
        ...
        ImmutableList.Builder<ModelRestrict> possibleInterruptions = new ImmutableList.Builder<>();
        while (iterator.advance()) {
    ...
    private ModelRestrict signalInterruptsSignalAtStartWhenSigsetMaskAllowsIt(
            int interruptingSignalTtid, long interruptingSignalNumber,
            ReadonlyEventInterface firstInterruptingSignalEvent, ReadonlyEventInterface lastInterruptingSignalEvent,
            Collection<ReadonlyEventInterface> interruptedThreadEvents,
            boolean enabled,
        ...
        ImmutableList.Builder<ModelRestrict> possibleInterruptionPlaces = new ImmutableList.Builder<>();
    ...
    private ModelRestrict signalInterruptsBetweenEvents(
        ...
        before.ifPresent(beforeEvent -> restricts.add(new Before(beforeEvent, firstSignalEvent)));
        after.ifPresent(afterEvent -> restricts.add(new Before(lastSignalEvent, afterEvent)));
        restricts.add(new SignalInterruptsThread(interruptingSignalTtid, interruptedTtid));
